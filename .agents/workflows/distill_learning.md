---
description: Breaks down a dense external source (article, paper, PDF, URL) into atomic, interlinked notes. Prioritizes augmenting existing Vault notes over creating duplicates to ensure knowledge synthesis. For lightweight saving, use /capture_content instead.
---
...
3. **Check for Existing Knowledge (Augmentation Check):**
   - For each extracted concept, search the Vault (using the `/ask_brain` workflow or `grep_search`) to determine if a note already exists that covers this topic.
...
4. **Draft or Augment Notes:**
   - For each concept identified in Step 2:
     - If no existing note was found: Draft a new atomic note following the `generate_obsidian_note` skill (proper YAML frontmatter, concise title, instructional format).
...
6. **Establish Connections (Intra-linking):**
   - Ensure the new or updated notes link back to the **source material** (URL or PDF).
...
8. **(Optional) Update Current Learning:**
   - If the distilled material represents an active area of study, add or update an entry in `Vault/1. The Core/1.1. Philosophy & Personal North Star/Current Learning.md` linking to the new or updated notes.